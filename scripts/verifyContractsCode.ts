import { DeployProgressCache, getDeployersAndConfig, loadDeployedContracts } from "./providerSetup";
...
class ContractVerifier {
  ...
  readonly contractToSource: { [key: string]: string } = {
  ...
  async verify(name: string, constructorArgs?: string) {
    ...
    const contractAddress = this.deployedContracts[name as keyof DeployProgressCache];
    if (!contractAddress) {
...
async function main() {
  ...
  const arbVerifier = new ContractVerifier(
    arbNetwork.chainID,
    process.env["ARBISCAN_API_KEY"] as string,
    deployedContracts
  );
  const arbDeployer = "0x0000000000000000000000000000000000000000"; // TODO - fill in
  await arbVerifier.verifyArbContracts(deployerConfig, arbDeployer);
  ...
  if (arbNetwork.partnerChainID == 1) {
    const novaVerifier = new ContractVerifier(
      novaNetwork.chainID,
      process.env["NOVA_ARBISCAN_API_KEY"] as string,
      deployedContracts
    );
