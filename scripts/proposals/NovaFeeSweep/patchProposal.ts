...
;(async () => {
  const proposalDataFile = process.argv[2]
  const proposalData = JSON.parse(fs.readFileSync(proposalDataFile).toString())
  const inputData = proposalData.arbSysSendTxToL1Args.calldata
  ...
  proposalData.arbSysSendTxToL1Args.calldata = patchedScheduleBatchCalldata

  fs.writeFileSync(proposalDataFile, JSON.stringify(proposalData, null, 2))
})()
