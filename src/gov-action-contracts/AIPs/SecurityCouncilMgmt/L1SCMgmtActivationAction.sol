...
contract L1SCMgmtActivationAction {
    ...
    function perform() external {
        ...
        require(
            !l1Timelock.hasRole(TIMELOCK_CANCELLER_ROLE, address(newEmergencySecurityCouncil)),
            "GovernanceChainSCMgmtActivationAction: mew emergency security council already has cancellor role"
        );

        l1Timelock.revokeRole(TIMELOCK_CANCELLER_ROLE, address(prevEmergencySecurityCouncil));
        l1Timelock.grantRole(TIMELOCK_CANCELLER_ROLE, address(newEmergencySecurityCouncil));
