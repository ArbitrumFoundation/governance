...
contract OfficeHoursAction {
    ...
    constructor(uint256 _minLocalHour, uint256 _maxLocalHour, int256 _localHourOffset, uint256 _minDayOfWeek, uint256 _maxDayOfWeek, uint256 _minimumTimestamp) {
    ...
    function perform() external view {
        ...
        require(weekday >= minDayOfWeek && weekday <= maxDayOfWeek, "Cannot execute outside the office days");

        // This is UTC time, leap seconds are not accounted for
        uint256 hoursSinceMidnight = (block.timestamp % 86400) / 3600;
        // Apply offset to convert to local time, also wrap if needed
        uint256 localHour = (hoursSinceMidnight + localHourOffset + 24) % 24;

        require(localHour >= minLocalHour && localHour < maxLocalHour, "Cannot execute outside of office hours");
    }
}
