...
contract ArbitrumVestingWallet is VestingWallet {
    ...
    function _vestingSchedule(
        ...
        if (timestamp < start()) {
            ...
            uint256 vestedTimeSecondsMonthFloored = vestedTimeSeconds -
                (vestedTimeSeconds % SECONDS_PER_MONTH);
            uint256 remaining = ((totalAllocation - cliff) * (vestedTimeSecondsMonthFloored)) /
                duration();
