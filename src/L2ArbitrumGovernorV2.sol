...
import {GovernorVotesQuorumFractionUpgradeable} from
    "openzeppelin-upgradeable-v5/governance/extensions/GovernorVotesQuorumFractionUpgradeable.sol";
import {GovernorPreventLateQuorumUpgradeable} from
    "openzeppelin-upgradeable-v5/governance/extensions/GovernorPreventLateQuorumUpgradeable.sol";
import {GovernorVotesUpgradeable} from
    "openzeppelin-upgradeable-v5/governance/extensions/GovernorVotesUpgradeable.sol";
import {TimelockControllerUpgradeable} from
    "openzeppelin-upgradeable-v5/governance/TimelockControllerUpgradeable.sol";
import {OwnableUpgradeable} from "openzeppelin-upgradeable-v5/access/OwnableUpgradeable.sol";
import {IVotes} from "openzeppelin-v5/governance/utils/IVotes.sol";
import {Address} from "openzeppelin-v5/utils/Address.sol";
    ...
    OwnableUpgradeable
{
    ...
    function getPastCirculatingSupply(uint256 timepoint) public view virtual returns (uint256) {
        return
            token().getPastTotalSupply(timepoint) - token().getPastVotes(EXCLUDE_ADDRESS, timepoint);
    }
    ...
    {
        return
            (getPastCirculatingSupply(timepoint) * quorumNumerator(timepoint)) / quorumDenominator();
    }
        ...
        returns (uint256)
    {
        return GovernorPreventLateQuorumUpgradeable._castVote(
            _proposalId, _account, _support, _reason, _params
        );
    }
        ...
        returns (uint256)
    {
        return GovernorTimelockControlUpgradeable._cancel(
            _targets, _values, _calldatas, _descriptionHash
        );
    }
