...
- **Timelock Salt for Non-emergency Security Council Proposals**
For typical governance proposals, the L2 timelock salt is computed on chain as the hash of the description text. For upgrades submitted directly in the L2 timelock (i.e., non-emergency security council proposals), this salt needs to be provided as a parameter. For ease of discoverability, the standard is for the L2 timelock salt to be equal to the L1 timelock salt in the given upgrade. (This is handled by the proposalBuilderCli script).  