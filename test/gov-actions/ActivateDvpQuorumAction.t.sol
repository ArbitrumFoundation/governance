...
import {ActivateDvpQuorumAction} from "../../../src/gov-action-contracts/AIPs/TotalDvpQuorum/ActivateDvpQuorumAction.sol";
...
contract ActivateDvpQuorumActionTest is Test {
    function testAction() external {
        ...
        assertEq(block.number, 23569916); // L1 block number corresponding to L2 block 389149842

        L2ArbitrumGovernor coreGovernor = L2ArbitrumGovernor(
            payable(0xf07DeD9dC292157749B6Fd268E37DF6EA38395B9)
        );
        L2ArbitrumGovernor treasuryGovernor = L2ArbitrumGovernor(
            payable(0x789fC99093B09aD01C34DC7251D0C89ce743e5a4)
        );

        uint256 prevCoreQuorum = coreGovernor.quorum(23569915);
        uint256 prevTreasuryQuorum = treasuryGovernor.quorum(23569915);
        assertEq(prevCoreQuorum, 212581618392648117373902586, "unexpected core quorum before upgrade");
        assertEq(prevTreasuryQuorum, 141721078928432078249268390, "unexpected treasury quorum before upgrade");

        address governorImpl = address(new L2ArbitrumGovernor());
        address tokenImpl = address(new L2ArbitrumToken());
        uint256 initialTDE = 5275963013349154829183295064 + 200_000_000 ether; // excluded + 200M
        ...
        ActivateDvpQuorumAction action = new ActivateDvpQuorumAction({
            ...
            _newCoreQuorumNumerator: 5_000, // 50%
            _coreMinimumQuorum: 250 ether,
            _coreMaximumQuorum: 250_000_000 ether,
            _newTreasuryQuorumNumerator: 4_000, // 40%
        ...
        IUpgradeExecutor(0xCF57572261c7c2BCF21ffD220ea7d1a27D40A827).execute(
            address(action),
            abi.encodeCall(ActivateDvpQuorumAction.perform, ())
        );
        ...
        assertEq(
            coreGovernor.quorum(23569915),
            prevCoreQuorum,
            "core governor quorum changed at previous block"
        );
        assertEq(
            treasuryGovernor.quorum(23569915),
            prevTreasuryQuorum,
        ...
        vm.roll(23569917);
        uint256 expectedCoreQuorum = (5_000 * 200_000_000 ether) / 10_000;
        uint256 expectedTreasuryQuorum = (4_000 * 200_000_000 ether) / 10_000;
        assertEq(
            coreGovernor.quorum(23569916),
            expectedCoreQuorum,
            "core governor quorum not calculated correctly at next block"
        );
        assertEq(
            treasuryGovernor.quorum(23569916),
